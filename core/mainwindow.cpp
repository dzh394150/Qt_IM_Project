            ...
            if (msgType == "chat_records") {
                ...
                if (chatModel) {
                    if (isLazyLoad) {
                        ...
                        // 【修复】先对消息按时间戳排序（升序，从旧到新）
                        std::sort(historyMessages.begin(), historyMessages.end(), [](const ChatMessage &a, const ChatMessage &b) {
                            return a.timestamp < b.timestamp;
                        });
                        
                        // 【修复】获取当前已显示消息的最小时间戳，用于去重
                        qint64 oldestExistingTs = 0;
                        QVector<ChatMessage> existingMessages = chatModel->messages();
                        if (!existingMessages.isEmpty()) {
                            // 找到最早的消息时间戳（排除系统消息）
                            for (const auto &existingMsg : existingMessages) {
                                if (existingMsg.owner != ChatMessage::System) {
                                    oldestExistingTs = existingMsg.timestamp.toMSecsSinceEpoch();
                                    break; // 消息已按时间正序排列，第一条就是最早的
                                }
                            }
                        }
                        
                        // 【修复】去重：只保存和添加本地模型中不存在的消息
                        QVector<ChatMessage> newMessages;
                        for (auto &msg : historyMessages) {
                            // 跳过时间戳大于等于现有最早消息的消息（这些消息已经在模型中了）
                            if (oldestExistingTs > 0 && msg.timestamp.toMSecsSinceEpoch() >= oldestExistingTs) {
                                qDebug() << "懒加载去重：跳过消息" << msg.timestamp.toString("yyyy-MM-dd hh:mm:ss") << msg.content.left(20);
                                continue;
                            }
                        ...
                        // 添加新消息到开头（新消息已按时间升序排列）
                        if (!newMessages.isEmpty()) {
                            chatModel->prependMessages(newMessages);
                        }
                        
                        // 恢复滚动位置：滚动到原来可见的第一项（现在索引增加了 newMessages.size()）
                        // 使用 QMetaObject::invokeMethod 确保在布局更新后执行
                        int newFirstVisibleRow = firstVisibleRow + newMessages.size();
                        QMetaObject::invokeMethod(this, [=] {
